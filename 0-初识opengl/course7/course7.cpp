...
float deltaTime = 0.0f;//当前帧与上一帧的帧差
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ///////////////////观察矩阵(也就是摄像机的矩阵)
        glm::mat4 view;
        /////////////lookat函数的参数：1.摄像机位置  2.目标位置（摄像机要看的“look at”对象） 3.世界空间中，表示向上位置的向量
        view = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
        ///////////////////投影矩阵
        glm::mat4 projection(1.0f);
        projection = glm::perspective(glm::radians(fov), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);//强转float
        ////////////////////////////////传入uniform
...
void processInput(GLFWwindow* window)
{
    if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
        ...
        ///////////////////下面这两个就是左右移动，但是要先获得一个右向量
        if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
            cameraPos -= glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
        if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
            cameraPos += glm::normalize(glm::cross(cameraFront, cameraUp)) * cameraSpeed;
        //监听鼠标的可见性，改变鼠标的可见性，方便后续的调试
